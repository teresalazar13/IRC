...
void process_client(int client_fd, unsigned long endereco, unsigned short porto);
void erro(char *msg);
...
int main() {
  ...
  while (1) {
    ...
    if (client > 0) {
      if (fork() == 0) {
        close(fd);
        process_client(client, client_addr.sin_addr.s_addr, client_addr.sin_port);
        exit(0);
      }
      close(client);
    }
...
void process_client(int client_fd, unsigned long endereco, unsigned short porto) {
  printf("Endereço IPv4: %lu\n", endereco );
  write(client_fd, (void*) endereco, sizeof(unsigned long));
  printf("PORTO %hu\n", porto );
  //write(client_fd, (void*) porto, sizeof(unsigned short));
  fflush(stdout);
  close(client_fd);
}
